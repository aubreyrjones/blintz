...
module BlintzGrammar
  ...
  def _nt__include
    start_index = index
    if node_cache[:_include].has_key?(index)
      cached = node_cache[:_include][index]
      if cached
        cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
        @index = cached.interval.end
      end
    ...
    node_cache[:_include][start_index] = r0
...
class BlintzGrammarParser < Treetop::Runtime::CompiledParser
  include BlintzGrammar
end
